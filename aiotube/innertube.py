    ...
    async def _call_api(self, endpoint, query, data):
        ...
        if self.use_oauth:
            if self.access_token:
                ...
                headers['Authorization'] = f'Bearer {self.access_token}'
        # response = asyncio.run(request.async_request(method="POST", url=endpoint_url, headers=headers, data=data))
        response = await request.async_request(method="POST", url=endpoint_url, headers=headers, data=data)
